...
void game_controller_init(void) {
	//allocate space for the tank position struct pointer
	tank_pos = (struct point_t*)malloc(sizeof(struct point_t));
	//allocate space for the tank bullet position struct pointer
	tank_bullet_pos = (struct point_t*)malloc(sizeof(struct point_t));
	//allocate space for the alien block position struct pointer
	alien_block_pos = (struct point_t*)malloc(sizeof(struct point_t));
	...
	xil_printf("ALIEN BLOCK POSITION %d %d\r\n", alien_block_pos->x, alien_block_pos->y);
...
void fire_alien_missile(void) {
	...
	alien_missiles[i].x = alien_block_pos->x + (alien_shooter % ALIEN_COLS)*ALIEN_WIDTH+ALIEN_MID;
	alien_missiles[i].y = alien_block_pos->y;
	alien_missiles[i].type = WEAK;

	xil_printf("MISSILE %d FIRED AT %d %d\r\n", i, alien_missiles[i].x, alien_missiles[i].y);
}
